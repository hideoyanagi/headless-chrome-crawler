...
class HCCrawler extends EventEmitter {
    ...
    return this._skipRequest(options)
      .then(skip => {
        if (skip) {
          ...
          return this._markRequested(options);
        }
        return this._markRequested(options)
          .then(() => this._followSitemap(options, depth))
          .then(() => this._request(options, depth))
          .then(links => {
            this._checkRequestCount();
            return this._followLinks(links, options, depth);
          })
  ...
  _skipRequest(options) {
    return Promise.all([
      ...
      this._checkAllowedPaths(options),
   ...
   * @private
   */
  _checkAllowedPaths(options) {
    // const { path } = parse(options.url);
    if (options.deniedPaths && checkPathMatch(options.deniedPaths, options.url)) return false;
    if (options.allowedPaths && !checkPathMatch(options.allowedPaths, options.url)) return false;
    return true;
  }
  /**
   * @param {!Object} options
  ...
  _checkRequested(options) {
    ...
    return this._cache.get(key).then(value => !!value);
  }
  ...
  _followLinks(urls, options, depth) {
    ...
    return Promise.all(map(urls, url => {
      const _options = extend({}, options, { url });
      return this._skipRequest(_options)
        .then(skip => {
          if (skip) return;
          this._push(_options, depth + 1);
        });
    }));
  }
