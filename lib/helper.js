...
class Helper {
  ...
  static resolveUrl(url, baseUrl) {
    // console.error('resolveUrl(', url, ', ', baseUrl, ')')
    url = trim(url);
    if (!url) return null;
  ...
  static checkDomainMatch(domains, hostname) {
    console.error('checkDomainMatch', domains, hostname)
   ...
   * @return {!boolean}
   */
  static checkPathMatch(allowedPaths, path) {
    console.error('checkPathMatch', allowedPaths, path)
    return some(allowedPaths, allowedPath => {
      if (isRegExp(allowedPath)) return allowedPath.test(path);
      // return domain === hostname;
      // return hostname.match(new RegExp(domain))
      return path.includes(alowedPath)
    });
  }
  /**
